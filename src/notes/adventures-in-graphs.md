...
I know that there's a neuropsychologically-rooted undderstanding of deep neural networks, but I am not a neuropsychologist, so my best intuitive grasp of how neural nets work is rooted in the [Universal Approximation Theorem](https://wikipedia.org/wiki/Universal_approximation_theorem) and functions in their most basic form, which is "you put a thing in, and you get a thing out". If we can clearly define what we're putting in and what we want to get out, then we can set up a model that takes thing `A` in and spits thing `B` out and let our training algorithms optimize that process ✨*mathemagically*✨ to do it with data it hasn't seen before. Thinking about a deep learning model this way, while not entirely rigorous or complex, has the added benefit of letting me focus on single idea, which helps me avoid getting bogged down in less-important implementation details or do something that's accidentally incompatible with the competition task.
...
For the sake of flexibility, I decided that, while the competition expects me to predict results for three years from the present, that number shouldn't necessarily be baked in to the fundamental design of the model, so I dubbed this the "n-Years Model", where `n = 3` in this case. 
